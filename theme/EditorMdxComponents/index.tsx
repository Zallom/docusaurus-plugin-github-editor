...
/**
 * Fallback preview for DocCard (needs DocsVersionProvider which doesn't exist on /edit).
 */
  ...
  return (
    <a
      ...
      <div style={{color: 'var(--ghe-text-primary, #fff)', fontWeight: 600, fontSize: '0.95rem'}}>
        {item.type === 'category' ? '\ud83d\uddc2\ufe0f ' : '\ud83d\udcc4 '}{item.label || 'Link'}
      </div>
      {item.description && (
...
function DocCardListPreview() {
  return (
    <div style={{
      ...
      Subpages list (DocCardList)
    </div>
  );
}

/**
 * Hook returning the default set of MDX components available in the editor preview.
 *
 * Swizzle `EditorMdxComponents` to add your own custom components:
 * ```
